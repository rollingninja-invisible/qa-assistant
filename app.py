...
if not api_key:
    st.error(
        "No Anthropic API key found. Please make sure ANTHROPIC_API_KEY is set in your .env file."
    )
    st.stop()

# Configure page
st.set_page_config(
    page_title="Script QA Assistant", layout="wide", initial_sidebar_state="expanded"
)
...
def extract_scene_characters(text):
    ...
    for match in matches:
        ...
        if (
            ...
            and len(name) > 1
            and not any(word in name for word in ["VOICES", "CROWD", "VARIOUS"])
            and not name.endswith("S HOME")
            and not name.startswith("CONT")
...
def validate_scene(scene_text, qa_row):
    ...
    validations["Scene Number"] = {
        ...
        "status": str(qa_row.get("Scene #", "")).strip()
        == scene_header["scene_number"].strip(),
    }
    ...
    validations["Has Multiple Setups"] = {
        ...
        "status": str(qa_row.get("Has Multiple Setups", "")).upper()
        == ("YES" if has_multiple else "NO"),
    }
    ...
    validations["Interior/Exterior"] = {
        "has_interior": {
            ...
            "status": str(qa_row.get("Has interior?", "")).upper()
            == ("YES" if is_int else "NO"),
        },
        "has_exterior": {
            "current": str(qa_row.get("Has exterior?", "")),
            "correct": "YES" if is_ext else "NO",
            "status": str(qa_row.get("Has exterior?", "")).upper()
    ...
    qa_characters = [
        char.strip()
        for char in str(qa_row.get("Characters Present", "")).split(",")
        if char.strip()
    ]
...
def format_validation_report(scene_number, validations):
    ...
    for key, value in validations.items():
        ...
        if (
            ...
            and "Characters" in key
            and (value.get("missing") or value.get("extra"))
        ):
            needs_corrections = True
            break
    ...
    if (
        not validations["Interior/Exterior"]["has_interior"]["status"]
        or not validations["Interior/Exterior"]["has_exterior"]["status"]
    ):
        report += "\nInterior/Exterior Settings\n"
...
with st.sidebar:
    ...
    qa_file = st.file_uploader(
        "Upload QA Sheet", type=["csv"], help="Upload the QA sheet in CSV format"
    )
                ...
                if script_text:
                    try:
                        ...
                        with st.expander("Analysis Details"):
                            ...
                            st.text(
                                f"Scene numbers: {', '.join(sorted(scenes.keys()))}"
                            )
                        ...
                        with tab1:
                            ...
                            for qa_row in qa_rows:
                                ...
                                with st.expander(f"Scene {scene_num}", expanded=True):
                                    if scene_content:
                                        validations = validate_scene(
                                            scene_content, qa_row
                                        )
                                        report = format_validation_report(
                                            scene_num, validations
                                        )
                                        st.markdown(report)
                                    else:
                        ...
                        st.success(
                            f"Analysis complete! Processed {len(scenes)} scenes."
                        )
